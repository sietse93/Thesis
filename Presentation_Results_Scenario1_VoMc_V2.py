...
plt.legend((neutral_1, neutral_2, neutral_3, no_reloc, no_map),
           ("Average RMSE value",
            "95% confidence interval",
            "Raw RMSE values",
            "ORB SLAM (place recognition bypassed)",
            "ORB SLAM (map point filtering bypassed)"), loc='upper left', fontsize=f_legend)

# #no_reloc = plt.plot([], [], color='blue', label="ORB no relocalization")
# #no_map = plt.plot([], [], color='red', label="ORB no map point cullling")
# plt.legend((no_reloc, no_map, plot_track_mc, plot_complete_mc),
#            ("ORB SLAM (place recognition bypassed)",
#             "ORB SLAM (map point filtering bypassed)",
#             "tracking failure",
