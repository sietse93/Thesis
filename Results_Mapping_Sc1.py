...
def main():
    ...
    statsc1_avg_rmse_trans, statsc1_std_rmse_trans, statsc1_avg_rmse_rot, statsc1_std_rmse_rot = \
        AvgStdRmseNoMapping(base_dir, town, starting_locations, "Stat", pose_estimation)

    sc2sc1_avg_rmse_trans, sc2sc1_std_rmse_trans, sc2sc1_avg_rmse_rot, sc2sc1_std_rmse_rot = \
        AvgStdRmseNoMapping(base_dir, town, starting_locations, "Sc2", pose_estimation)

    sc1sc1_avg_rmse_trans, sc1sc1_std_rmse_trans, sc1sc1_avg_rmse_rot, sc1sc1_std_rmse_rot = \
    ...
    # plt.figure("Pose estimation: Localization and mapping")
    # plt.subplot(2, 1, 1)
    # plt.bar(ind, np.array(maploc_avg_rmse_trans), width, yerr=2 * np.array(maploc_std_rmse_trans), color='black',
    #         label="Localization and mapping, Static")
    # plt.bar(ind + 1 * width, np.array(sc1_avg_rmse_trans), width, yerr=2 * np.array(sc1_std_rmse_trans),
    #         color='gold', label="Sc1 Pose estimation")
    # plt.bar(ind + 2*width, np.array(sc2_avg_rmse_trans), width, yerr=2 * np.array(sc2_std_rmse_trans),
    #         color='olivedrab', label="Sc2 Pose estimation")
    #
    # plt.subplot(2, 1, 2)
    ...
    plt.figure("Pose estimation: vehicle in front with predetermined map")
    plt.suptitle("Results town 1 vehicle in front: utilize map created under various conditions", fontsize=18, fontweight='bold', y=0.95)
    plt.subplot(2, 1, 1)
    plt.title("Translational component root mean square error of the relative pose error", fontsize=fs_title, fontweight='bold')
    plt.bar(ind, np.array(maploc_avg_rmse_trans), width, yerr=2*np.array(maploc_std_rmse_trans), color='black')
    plt.bar(ind+1*width, np.array(statsc1_avg_rmse_trans), width,  yerr=2*np.array(statsc1_std_rmse_trans), color='sienna')
    plt.bar(ind + 2 * width, np.array(sc1sc1_avg_rmse_trans), width, yerr=2 * np.array(sc1sc1_std_rmse_trans),
            color='gold', label="Pose = vehicle in front, map = vehicle in front")
    plt.bar(ind + 3*width, np.array(sc2sc1_avg_rmse_trans), width, yerr=2 * np.array(sc2sc1_std_rmse_trans),
            color='olivedrab')
    ...
    plt.bar(ind, np.array(maploc_avg_rmse_rot), width, yerr=2 * np.array(maploc_std_rmse_rot), color='black')
    plt.bar(ind + 1 * width, np.array(statsc1_avg_rmse_rot), width, yerr=2 * np.array(statsc1_std_rmse_rot),
            color='sienna')
    plt.bar(ind + 2 * width, np.array(sc1sc1_avg_rmse_rot), width, yerr=2 * np.array(sc1sc1_std_rmse_rot),
            color='gold')
    plt.bar(ind + 3 * width, np.array(sc2sc1_avg_rmse_rot), width, yerr=2 * np.array(sc2sc1_std_rmse_rot),
            color='olivedrab')
    plt.ylim(bottom=0)
    ...
    legend_slam = mpatches.Patch(color='black', label='"Vehicle in front: simultaneous localization and mapping"')
    legend_static = mpatches.Patch(color='sienna', label="Pose = vehicle in front, map = static")
    legend_sc1 = mpatches.Patch(color='gold', label="Pose = vehicle in front, map = vehicle in front")
    legend_sc2 = mpatches.Patch(color='olivedrab', label="Pose = vehicle in front, map = vehicles opposite lane")
...
def AvgStdRmseNoMapping(base_dir, town, starting_locations, map_estimation, pose_estimation):
    ...
    for SL in starting_locations:
        ...
        if map_estimation == "Stat" and SL == 58 and pose_estimation == "Sc1":
            rmse_trans_list.pop(0)
        rmse_trans_all = np.array(rmse_trans_list)
        avg_rmse_trans = np.mean(rmse_trans_all)
        avg_rmse_trans_list.append(avg_rmse_trans)
        std_rmse_trans = np.std(rmse_trans_all)
