...
def main():
    ...
    statsc2_avg_rmse_trans, statsc2_std_rmse_trans, statsc2_avg_rmse_rot, statsc2_std_rmse_rot = \
        AvgStdRmseNoMapping(base_dir, town, starting_locations, "Stat", pose_estimation)

    sc2sc2_avg_rmse_trans, sc2sc2_std_rmse_trans, sc2sc2_avg_rmse_rot, sc2sc2_std_rmse_rot = \
        AvgStdRmseNoMapping(base_dir, town, starting_locations, "Sc2", pose_estimation)

    sc1sc2_avg_rmse_trans, sc1sc2_std_rmse_trans, sc1sc2_avg_rmse_rot, sc1sc2_std_rmse_rot = \
    ...
    plt.figure("Pose estimation: vehicles opposite lane with predetermined map")
    plt.suptitle("Results town 1 vehicles opposite lane: utilize map created under various conditions", fontsize=18, fontweight='bold', y=0.95)
    plt.subplot(2, 1, 1)
    plt.title("Translational component root mean square error of the relative pose error", fontsize=fs_title, fontweight='bold')
    plt.bar(ind, np.array(maploc_avg_rmse_trans), width, yerr=2*np.array(maploc_std_rmse_trans), color='black')
    plt.bar(ind+1*width, np.array(statsc2_avg_rmse_trans), width,  yerr=2*np.array(statsc2_std_rmse_trans), color='sienna')
    plt.bar(ind + 2 * width, np.array(sc1sc2_avg_rmse_trans), width, yerr=2 * np.array(sc1sc2_std_rmse_trans),
            color='gold')
    plt.bar(ind + 3*width, np.array(sc2sc2_avg_rmse_trans), width, yerr=2 * np.array(sc2sc2_std_rmse_trans),
            color='olivedrab')
    ...
    plt.bar(ind, np.array(maploc_avg_rmse_rot), width, yerr=2 * np.array(maploc_std_rmse_rot), color='black')
    plt.bar(ind + 1 * width, np.array(statsc2_avg_rmse_rot), width, yerr=2 * np.array(statsc2_std_rmse_rot),
            color='sienna')
    plt.bar(ind + 2 * width, np.array(sc1sc2_avg_rmse_rot), width, yerr=2 * np.array(sc1sc2_std_rmse_rot),
            color='gold')
    plt.bar(ind + 3 * width, np.array(sc2sc2_avg_rmse_rot), width, yerr=2 * np.array(sc2sc2_std_rmse_rot),
            color='olivedrab')
    plt.ylim(bottom=0)
    ...
    legend_slam = mpatches.Patch(color='black', label="vehicles opposite lane: simultaneous localization and mapping")
    legend_static = mpatches.Patch(color='sienna', label="Pose = vehicles opposite lane, map = static")
    legend_sc1 = mpatches.Patch(color='gold', label="Pose = vehicles opposite lane, map = vehicle in front")
    legend_sc2 = mpatches.Patch(color='olivedrab', label="Pose = vehicles opposite lane, map = vehicles opposite lane")
