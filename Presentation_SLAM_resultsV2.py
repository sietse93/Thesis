...
ind = np.arange(len(x[0:4]))

plt.subplot(1, 1, 1)
plt.plot(ind[0:4], np.array(avg_trans[0:4]), color='green', label="Average RMSE Value")
plt.fill_between(ind[0:4], np.array(avg_trans_low[0:4]), np.array(avg_trans_upp[0:4]), color='green', alpha=0.5, label="95% confidence interval")
...
plt.ylim(bottom=0.010, top=0.060)
