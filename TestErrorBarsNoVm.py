...
def main():
    ...
    for index, SL in enumerate(starting_locations):
        ...
        for ds in dynamic_scenarios:
            ...
            orb_static_novm, gt_novm = InspectJsonFileInDir(Town, SL, base_dir, dir_name_stat, modevm)
            orb_dynamic_novm, gt_novm = InspectJsonFileInDir(Town, SL, base_dir, dir_name_dyn, modevm)
            PerfNovm = ScenarioLocationPerformance(ds, Town, SL, orb_static_novm, orb_dynamic_novm, gt_novm)
            ...
            if ds == 20:
                ...
                avg_trans_novm.extend([PerfNovm.rmse_static_avg[0], PerfNovm.rmse_dynamic_avg[0]])
                std_trans_novm.extend([PerfNovm.rmse_static_std[0], PerfNovm.rmse_dynamic_std[0]])
                avg_rot_novm.extend([PerfNovm.rmse_static_avg[1], PerfNovm.rmse_dynamic_avg[1]])
                std_rot_novm.extend([PerfNovm.rmse_static_std[1], PerfNovm.rmse_dynamic_std[1]])
                ...
                avg_trans_novm.append(PerfNovm.rmse_dynamic_avg[0])
                std_trans_novm.append(PerfNovm.rmse_dynamic_std[0])
                avg_rot_novm.append(PerfNovm.rmse_dynamic_avg[1])
                std_rot_novm.append(PerfNovm.rmse_dynamic_std[1])
        # have to loop it otherwise you get the same color
        plt.subplot(3, 1, 1)
        # plt.grid(True)
        label_location = "T" + str(Perf.location["Town"]) + "Nr" + str(index+1)
        plt.errorbar(ind[i:i+(len(dynamic_scenarios)+1)], np.array(avg_trans), np.array(std_trans), color='green', label="VO")
        plt.errorbar(ind[i:i+(len(dynamic_scenarios)+1)], np.array(avg_trans_novm), np.array(std_trans_novm), color='red', label="NoVm")
        plt.subplots_adjust(hspace=0.5)
        plt.legend()

        plt.subplot(3, 1, 2)
        label_location = "T" + str(Perf.location["Town"]) + "Nr" + str(index + 1)
        plt.errorbar(ind[i:i + (len(dynamic_scenarios) + 1)], np.array(avg_rot), np.array(std_rot), color='green', label="VO")
        plt.errorbar(ind[i:i + (len(dynamic_scenarios) + 1)], np.array(avg_rot_novm), np.array(std_rot_novm), color='red', label="NoVm")
        plt.legend()
