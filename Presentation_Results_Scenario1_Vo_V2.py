...
f_legend = 12
fs_title = 16
x_lim_right = len(x[0:4])-0.5
x_lim_left = -0.5

plt.figure("Performance SLAM Algorithm")
plt.suptitle(r'Root Mean Square Error of Relative Pose Error ($\Delta$ = 100m ) - Stuck behind van', fontsize=18, fontweight='bold', y=0.95)
ind = np.arange(len(x))

plt.subplot(1, 1, 1)
# for j in range(len(starting_locations[0])):
j = 0
...
neutral_1 = Line2D([0], [0], color='blue')
neutral_2 = plt.fill_between([], [], [], color="blue", alpha=0.5, label="95% confidence interval")
neutral_3 = plt.scatter([], [], color='blue', marker='x', label="Raw RMSE values")

plt.legend((neutral_1, neutral_2, neutral_3),
           ("Average RMSE value (bypassed place recognition)",
            "95% confidence interval",
            "Raw RMSE values"), loc='upper left', fontsize=f_legend)
...
plt.title("Translational component", fontsize=fs_title, fontweight='bold')
plt.ylabel("Trans. RMSE RPE [m/m]", fontsize=fs_y)
plt.xticks(ind[0:4], x[0:4], fontsize=fs_x)
...
# plt.grid(True, linestyle='--', alpha=0.5)
#
# neutral_1 = Line2D([0], [0], color='black')
# neutral_2 = plt.fill_between([], [], [], color="grey", alpha=0.5, label="95% confidence interval")
# neutral_3 = plt.scatter([], [], color='black', marker='x', label="Raw RMSE values")
# no_reloc = Line2D([0], [0], color='blue')
# no_map = Line2D([0], [0], color='red')
# #no_reloc = plt.plot([], [], color='blue', label="ORB no relocalization")
# #no_map = plt.plot([], [], color='red', label="ORB no map point cullling")
# plt.legend((plot_track, plot_complete),
#            ("tracking failure",
#             "successful tracking"), loc='upper left', fontsize=f_legend)
# #plt.legend((plot_track_mc, plot_complete_mc), ("tracking failure", "successful tracking"), loc='upper left', fontsize=f_legend)
# plt.ylim(top=110.0)
# plt.xlim(left=x_lim_left, right=x_lim_right)
