...
for orb_stat, orb_dyn, orb_dyn_mc in zip(orb_static_list, orb_dynamic_list, orb_dynamic_list_mc):
    ...
    plt.plot(dyn_time, dyn_trans_err, color='red')
    # plt.plot(dyn_time_mc, dyn_trans_err_mc, color='red')

    plt.subplot(2, 1, 2)
    plt.plot(stat_time, stat_rot_err, color='black')
    plt.plot(dyn_time, dyn_rot_err, color='red')
...
plt.subplot(2,1,1)
plt.ylim(top=0.8)
plt.suptitle(r'ORB SLAM Relative Pose Error ($\Delta$ = 10m) over time', fontsize=18, fontweight='bold', y=0.95)
...
plt.plot([], [], color='black', label="Static")
plt.plot([], [], color='red', label="Vans driving opposite road")
